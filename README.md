...
| NRG Haus | `GOOGLE_SHEETS_SPREADSHEET_ID_NRG_HAUS` (ID pending) | `I3` | `H46` |
...
## Finishing NRG Haus setup

Cell mapping (`I3` goal, `H46` revenue MTD) is already configured in `src/config/businesses.ts`. Still needed:

1. Get the spreadsheet's ID from its URL and share it with the service account email as Viewer.
2. Set `GOOGLE_SHEETS_SPREADSHEET_ID_NRG_HAUS` in the environment — the card will switch from "Not connected" to live data automatically, no code changes needed.
3. Drop the real logo file into `public/logos/nrg-haus.svg` (or `.png`) and flip `logoKind` to `"image"` in the same config entry — until then it renders as a text wordmark placeholder.
